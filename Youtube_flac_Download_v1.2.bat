...
set "yt_title=!yt_title:<=＜!"
set "yt_title=!yt_title:>=＞!"
for /f "delims=* tokens=1,2" %%i in ("%yt_title%") do set BB=%%i %%j
set yt_title=%BB%
REM echo flg >%tmp%\!yt_title!.ytflg
REM set PARAM=ytflg
REM for /f "usebackq" %%A in (`dir %tmp% /b ^| find "%PARAM%"`) do set FILE=%%A
REM set "FILE=%FILE%"
REM set "AFTER=!FILE:.ytflg=!"
REM set yt_title=!AFTER!
REM del /s /q %tmp%\!yt_title!.ytflg
echo ファイル名：%yt_title%
...
set PARAM2=.webm
for /f "usebackq" %%B in (`dir %dl_dir% /b ^| find "%PARAM2%"`) do set WebM=%%B
echo WebMファイル名：WebM%

set PARAM3=.png
for /f "usebackq" %%C in (`dir %dl_dir% /b ^| find "%PARAM3%"`) do set pngdata=%%C
echo pngファイル名：%pngdata%
...
rmdir /s /q "%dl_dir%\99_extract_original"
